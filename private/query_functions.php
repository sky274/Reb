<?php
  ...
  function validate_state($state, $errors=array()) {
    ...
    if (is_blank($state['name'])) {
     ...
     if (is_blank($state['code'])) {
      ...
      $errors[] = "State code can only contain letters.";
    }  
    }
    return $errors;
  }
  ...
  function insert_state($state) {
    ...
    if ($stmt = mysqli_prepare($db, "INSERT INTO states (name, code) VALUES(?, ?)")) {
      $stmt->bind_param("ss", $state['name'], $state['code']);
      $stmt->execute();
      return true;
    }
    else {
  ...
  function validate_territory($territory, $errors=array()) {
    // TODO add validations
     ...
     if (is_blank($territory['name'])) {
      ...
      $errors[] = "Territory must be between 1 and 255 characters.";
    } elseif (preg_match('/\A[A-Za-z\s]+\Z/', $territory['name']) == 0) {
      $errors[] = "Territory can only contain letters.";
    } 
  ...
  function validate_salesperson($salesperson, $errors=array()) {
    ...
    if(is_blank($salesperson['phone'])){
      ...
      $errors[]= "phone number has to be in valid format";
    }
  ...
  function insert_salesperson($salesperson) {
    ...
    // code from https://www.w3schools.com/php/php_mysql_prepared_statements.asp prepared statements
    if ($stmt = $db->prepare( "INSERT INTO salespeople (first_name, last_name, phone, email) VALUES (?, ?, ?, ?)")) {
